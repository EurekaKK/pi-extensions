...
- 默认测试必须停留在 Harbor 可见的 trial launch plan：JobConfig YAML 与假 environment 上的 `install` / `run` / `populate_context_post_run`。不要为 bash driver 另开产品 seam，也不要在默认测试里跑 Docker、真实模型或用户密钥。
