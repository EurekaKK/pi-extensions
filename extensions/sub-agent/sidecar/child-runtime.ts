...
import type {
	BootstrapFrame,
	DegradedExtensionCodeV1,
	DegradedExtensionV1,
	RunFailureCodeV1,
	SpawnFrame,
	ThinkingLevelV1,
	UnavailableToolV1,
	WorkerOperationErrorCode,
} from "./protocol.js";
...
type SettingsSnapshot = Parameters<PiSdkModule["SettingsManager"]["inMemory"]>[0];
...
const OPERATION_FAILURE_CODES = new Set<WorkerOperationErrorCode>([
...
export class ChildRuntimeError extends Error {
	constructor(
		readonly code: WorkerOperationErrorCode,
		message: string = code,
	) {
		super(message);
...
export type ChildTerminalCandidate =
	| { outcome: "RESULT"; report: string }
	| { outcome: "FAILED"; failureCode: RunFailureCodeV1 };
...
export interface ChildRunCallbacks {
	onAccepted(): Promise<void>;
	onTerminal(candidate: ChildTerminalCandidate): Promise<void>;
}

export type ChildRunAdmission =
	| { status: "accepted" }
	| { status: "rejected"; code: "SUBAGENT_INPUT_HANDLED" | "SUBAGENT_RUN_START_FAILED" };

export interface ChildRuntimeLifecycleHooks {
	onToolStateChanged(activeToolCount: number, capabilityToolCount: number): void;
	onWarning(code: string, runId?: string): void;
}

export interface ChildRuntime {
	readonly model: { provider: string; id: string };
	readonly thinkingLevel: ThinkingLevelV1;
	readonly activeToolCount: number;
	readonly capabilityToolCount: number;
	readonly degradedExtensions: readonly DegradedExtensionV1[];
	readonly unavailableTools: readonly UnavailableToolV1[];
	startRun(request: ChildRunRequest, callbacks: ChildRunCallbacks): Promise<ChildRunAdmission>;
...
export interface ChildRuntimeFactory {
	...
	create(frame: SpawnFrame, hooks: ChildRuntimeLifecycleHooks): Promise<ChildRuntime>;
}

interface LoadedPi {
	sdk: PiSdkModule;
	ai: Pick<PiAiModule, "getSupportedThinkingLevels">;
	version: string;
}
...
interface ActiveRun {
	...
	callbacks: ChildRunCallbacks;
	accepted: boolean;
	gateOpen: boolean;
	cancelRequested: boolean;
	finalCompactionFailure: boolean;
	resolveAdmission: (result: ChildRunAdmission) => void;
...
async function loadJson(path: string): Promise<Record<string, unknown>> {
	...
	if (!isRecord(parsed)) return throwUnsupported("Pi package metadata is not an object.");
	return parsed;
}

function assertContainedFile(root: string, candidate: string): void {
...
async function importLoadedPi(piPackageDir: string): Promise<LoadedPi> {
	...
	try {
		...
		let piAiPackage: Record<string, unknown> | undefined;
	...
	return {
		sdk: sdkValue as PiSdkModule,
		ai: aiValue as Pick<PiAiModule, "getSupportedThinkingLevels">,
		version,
	};
}

function settingsSnapshot(frame: BootstrapFrame): SettingsSnapshot {
	return structuredClone(frame.settingsSnapshot) as unknown as SettingsSnapshot;
}

function resourceLoaderOptions(candidatePaths: readonly string[], guardFactory?: ExtensionFactory) {
	return {
		noExtensions: true,
...
function assertSessionSurface(result: CreateAgentSessionResult): void {
	const { session, extensionsResult } = result;
	const sessionRecord = session as unknown as Record<string, unknown>;
	const sessionManagerRecord = session.sessionManager as unknown as Record<string, unknown>;
	const runtimeRecord = extensionsResult.runtime as unknown as Record<string, unknown>;
	for (const method of [
		"bindExtensions",
		"subscribe",
		"prompt",
		...
		if (!hasFunction(sessionRecord, method)) throwUnsupported(`Pi AgentSession.${method}() is unavailable.`);
	}
...
async function runCompatibilityProbe(loaded: LoadedPi, bootstrap: BootstrapFrame): Promise<void> {
	...
	try {
		...
		if (
			result.session.resourceLoader.getSkills().skills.length !== 0 ||
		...
		if (error instanceof ChildRuntimeError) throw error;
		throw new ChildRuntimeError("SUBAGENT_PI_API_UNSUPPORTED", "Pi Worker compatibility probe failed.");
	} finally {
		if (result) await shutdownSession(result.session).catch(() => undefined);
	}
}
...
async function canonicalCandidates(frame: SpawnFrame, bootstrap: BootstrapFrame): Promise<string[]> {
...
async function canonicalSet(paths: readonly string[]): Promise<Set<string>> {
	return new Set(await Promise.all(paths.map(canonicalPath)));
}

function loaderFailureCode(message: string): DegradedExtensionCodeV1 {
	if (
		message.includes("does not export a valid factory") ||
...
function collectServiceFailures(services: AgentSessionServices): ExtensionFailure[] {
	const failures: ExtensionFailure[] = services.resourceLoader
		.getExtensions()
...
function bindFailure(error: ExtensionError): ExtensionFailure | undefined {
...
function dedupeFailures(failures: readonly ExtensionFailure[]): ExtensionFailure[] {
	const byPath = new Map<string, ExtensionFailure>();
	for (const failure of failures) {
		if (!byPath.has(failure.path)) byPath.set(failure.path, failure);
	}
	return [...byPath.values()];
}

function projectContextText(frame: SpawnFrame): string | undefined {
	if (frame.projectContext.length === 0) return undefined;
	return frame.projectContext.map((entry) => `--- ${entry.path} ---\n${entry.content}`).join("\n\n");
}

function buildTransient(frame: SpawnFrame, run: ActiveRun): string {
	const runtime = `[SUBAGENT_RUNTIME_STATE v=1 agentId=${JSON.stringify(frame.agentId)} runId=${JSON.stringify(
		run.request.runId,
	)}]
cancelRequested=${run.cancelRequested ? "true" : "false"}
[/SUBAGENT_RUNTIME_STATE]`;
	const projectContext = projectContextText(frame);
	if (projectContext === undefined) return runtime;
	return `${runtime}
...
function isCompactionFailure(event: AgentSessionEvent): boolean {
	return event.type === "compaction_end" && !event.willRetry && event.result === undefined;
}

function classifyTerminal(session: AgentSession, finalCompactionFailure: boolean): ChildTerminalCandidate {
	...
	if (!assistant) return { outcome: "FAILED", failureCode: "SUBAGENT_EMPTY_RESULT" };
	...
	for (const block of assistant.content) {
		...
		if (block.text.length > 0) hasNonEmptyText = true;
	}
	if (!hasNonEmptyText) return { outcome: "FAILED", failureCode: "SUBAGENT_EMPTY_RESULT" };
...
class GuardBridge {
	...
	constructor(
		readonly frame: SpawnFrame,
		readonly hooks: ChildRuntimeLifecycleHooks,
	) {}
	...
	guardFactory(): ExtensionFactory {
		return (pi) => {
			pi.on("context", (event) => {
				...
				if (!run) return { messages: event.messages };
	...
	resolveAdmission(run: ActiveRun, result: ChildRunAdmission): void {
		if (run.admissionResolved) return;
	...
	rejectMessageAction(): void {
		if (this.binding) throw new Error(GATE_REJECTION_MESSAGE);
...
class PiChildRuntime implements ChildRuntime {
	readonly degradedExtensions: DegradedExtensionV1[] = [];
	readonly unavailableTools: UnavailableToolV1[] = [];
	activeToolCount = 0;
	...
	constructor(
		...
		this.unsubscribe = session.subscribe((event) => {
			...
			if (run && isCompactionFailure(event)) run.finalCompactionFailure = true;
		});
	}
	...
	promote(
		...
		degraded: readonly DegradedExtensionV1[],
		unavailable: readonly UnavailableToolV1[],
	): void {
	...
	async startRun(request: ChildRunRequest, callbacks: ChildRunCallbacks): Promise<ChildRunAdmission> {
		if (this.shutdownPromise || this.bridge.activeRun || !this.session.isIdle) {
			return { status: "rejected", code: "SUBAGENT_RUN_START_FAILED" };
		}
		let resolveAdmission!: (result: ChildRunAdmission) => void;
		const admission = new Promise<ChildRunAdmission>((resolveResult) => {
		...
		void this.session
			...
			.then(() => {
				if (!run.accepted) {
					...
					if (this.bridge.activeRun === run) this.bridge.activeRun = undefined;
				}
			})
			.catch(() => {
				if (!run.accepted) {
	...
	closeRunAdmissionGate(runId: string): void {
		const run = this.bridge.activeRun;
		if (!run || run.request.runId !== runId) return;
		run.gateOpen = false;
...
export class PiChildRuntimeFactory implements ChildRuntimeFactory {
	...
	private constructor(
		private readonly loaded: LoadedPi,
		private readonly bootstrapFrame: BootstrapFrame,
	) {
		this.piVersion = loaded.version;
	}

	static async bootstrap(frame: BootstrapFrame): Promise<PiChildRuntimeFactory> {
	...
	async create(frame: SpawnFrame, hooks: ChildRuntimeLifecycleHooks): Promise<ChildRuntime> {
		...
		return await this.finalizeRuntime(
			frame,
			second.runtime,
			firstFailures.map((failure) => ({ ...failure })),
			failedPaths,
		);
	}
	...
	private async createAttempt(
		frame: SpawnFrame,
		hooks: ChildRuntimeLifecycleHooks,
		...
		try {
			...
			if (!model) throw new ChildRuntimeError("SUBAGENT_MODEL_NOT_FOUND");
			const hasAuth =
				services.modelRuntime.hasConfiguredAuth(frame.model.provider) ||
				(await services.modelRuntime.checkAuth(frame.model.provider)) !== undefined;
			if (!hasAuth) throw new ChildRuntimeError("SUBAGENT_MODEL_AUTH_REQUIRED");
			...
			const capabilityNames = [
				...new Set(
					(frame.explicitTools ?? frame.parentToolNames).filter((name) => !RESERVED_CHILD_TOOL_NAMES.has(name)),
				),
			];
			...
			installRuntimeGuards(
				...
				result.session,
				bridge,
				result.session.getAllTools().length,
			);
			const bindErrors: ExtensionError[] = [];
			...
			const failures = [
				...serviceFailures,
				...bindErrors.map(bindFailure).filter((failure): failure is ExtensionFailure => failure !== undefined),
			];
			...
			throw new ChildRuntimeError(
				"SUBAGENT_PI_API_UNSUPPORTED",
				`Child runtime initialization failed: ${asErrorMessage(error)}`,
			);
		}
	}
	...
	private async finalizeRuntime(
		...
		const unavailable: UnavailableToolV1[] =
			frame.explicitTools === undefined
				? await Promise.all(
						missing.map(async (name) => {
							const sourcePath = sourceByName.get(name);
							if (sourcePath === undefined) return { name, reason: "unlocatable" as const };
							const canonicalSource = await canonicalPath(sourcePath);
							if (failedPaths.has(canonicalSource)) return { name, reason: "extension_failed" as const };
							return { name, reason: "not_registered" as const };
						}),
					)
				: [];
