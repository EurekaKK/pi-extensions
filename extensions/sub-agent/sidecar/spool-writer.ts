...
import type { ResultSpoolMetadata } from "./protocol.js";
...
function spoolBasename(deliveryId: string): string {
...
async function trustedSpoolRoot(spoolDir: string): Promise<string> {
...
export async function writeResultSpool(
	spoolDir: string,
	deliveryId: string,
	report: string,
): Promise<ResultSpoolMetadata> {
	...
	if (dirname(finalPath) !== root) throw new Error("Result spool path escapes its root.");
	const temporaryPath = resolve(root, `.${finalBasename}.${process.pid}.${randomUUID()}.tmp`);
	if (dirname(temporaryPath) !== root) throw new Error("Temporary result spool path escapes its root.");
...
export async function removeResultSpoolFile(spoolDir: string, deliveryId: string): Promise<void> {
	const root = await trustedSpoolRoot(spoolDir);
