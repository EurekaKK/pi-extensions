...
import {
	type AgentToolStateFrame,
	type BootstrapFrame,
	type CancelAcceptedFrame,
	type ChildFrame,
	type ChildModelSelection,
	type DegradedExtensionV1,
	type GuardianReadyFrame,
	isControlMessageWithinLimit,
	isPlainRecord,
	type KillAcceptedFrame,
	type KillSettledFrame,
	matchesProtocolIdentity,
	type OperationNackFrame,
	type ParentFrame,
	type ProtocolEnvelope,
	type ProtocolErrorFrame,
	type ResultSpoolMetadata,
	type RunAcceptedFrame,
	type RunSettledFrame,
	type RunTerminalFrame,
	type ShutdownFrame,
	SUBAGENT_PROTOCOL_VERSION,
	type UnavailableToolV1,
	type WorkerReadyFrame,
	type WorkerWarningFrame,
} from "./protocol.js";
...
type GuardianPhase = "BOOTSTRAP" | "RUNNING" | "LOSS_CLEANUP" | "SHUTDOWN" | "DONE";
type ProtocolIdentity = Pick<ProtocolEnvelope, "sessionNonce" | "managerEpoch" | "workerGeneration">;

interface SpoolIdentity {
...
function isModelSelection(value: unknown): value is ChildModelSelection {
	return (
		isPlainRecord(value) &&
...
function isParentToolSources(value: unknown): boolean {
	return (
		Array.isArray(value) &&
		value.every(
			(entry) =>
...
function isBootstrapFrameStrict(value: unknown): value is BootstrapFrame {
	return (
		isPlainRecord(value) &&
		hasExactKeys(value, [
			"protocolVersion",
...
function isSpawnFrameStrict(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(
			value,
			[
...
function isSendFrameStrict(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(value, [
			"protocolVersion",
			"sessionNonce",
...
function isCancelFrameStrict(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(value, [
			"protocolVersion",
			"sessionNonce",
...
function isKillFrameStrict(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(value, [
			"protocolVersion",
			"sessionNonce",
...
function isMailboxCommitAckFrameStrict(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(value, [
			"protocolVersion",
			"sessionNonce",
...
function isShutdownFrameStrict(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(value, [
			"protocolVersion",
			"sessionNonce",
...
export function isStrictParentFrame(value: unknown): value is ParentFrame {
	if (!isPlainRecord(value) || !hasEnvelope(value) || !isControlMessageWithinLimit(value)) return false;
	switch (value.type) {
		case "BOOTSTRAP":
...
function isDegradedExtension(value: unknown): value is DegradedExtensionV1 {
	return (
		isPlainRecord(value) &&
...
function isUnavailableTool(value: unknown): value is UnavailableToolV1 {
	return (
		isPlainRecord(value) &&
...
function isRunAcceptedFrame(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(value, [
			"protocolVersion",
			"sessionNonce",
...
function isOperationNackFrame(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(
			value,
			[
...
function isCancelAcceptedFrame(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(value, [
			"protocolVersion",
			"sessionNonce",
...
function isKillAcceptedFrame(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(value, [
			"protocolVersion",
			"sessionNonce",
...
function isKillSettledFrame(value: Record<string, unknown>): boolean {
	return isKillAcceptedFrame({ ...value, type: "KILL_ACCEPTED" }) && value.type === "KILL_SETTLED";
}

function isResultSpoolMetadata(value: unknown): value is ResultSpoolMetadata {
	return (
		isPlainRecord(value) &&
...
function hasRunTerminalBase(value: Record<string, unknown>): boolean {
	return (
...
function isRunTerminalFrame(value: Record<string, unknown>): boolean {
	const baseKeys = [
		"protocolVersion",
		"sessionNonce",
		"managerEpoch",
	...
	if (!hasRunTerminalBase(value)) return false;
	if (value.outcome === "RESULT") {
		return hasExactKeys(value, [...baseKeys, "spool"]) && isResultSpoolMetadata(value.spool);
	}
	if (value.outcome === "FAILED") {
		return (
			hasExactKeys(value, [...baseKeys, "failureCode"]) &&
			isNonEmptyString(value.failureCode) &&
			RUN_FAILURE_CODES.has(value.failureCode)
		);
	}
	return (
		value.outcome === "CANCELLED" && hasExactKeys(value, [...baseKeys, "cancelReason"]) && isString(value.cancelReason)
	);
}

function isRunSettledFrame(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(value, [
			"protocolVersion",
			"sessionNonce",
...
function isAgentToolStateFrame(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(value, [
			"protocolVersion",
			"sessionNonce",
...
function isWorkerWarningFrame(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(
			value,
			["protocolVersion", "sessionNonce", "managerEpoch", "workerGeneration", "type", "code"],
			["agentId", "runId"],
		) &&
...
function isWorkerProtocolErrorFrame(value: Record<string, unknown>): boolean {
	return (
		hasExactKeys(value, ["protocolVersion", "sessionNonce", "managerEpoch", "workerGeneration", "type", "code"]) &&
		value.type === "PROTOCOL_ERROR" &&
		(value.code === "SUBAGENT_PROTOCOL_MISMATCH" || value.code === "SUBAGENT_PI_API_UNSUPPORTED")
	);
}

export function isStrictWorkerFrame(value: unknown): value is WorkerFrame {
	if (!isPlainRecord(value) || !hasEnvelope(value) || !isControlMessageWithinLimit(value)) return false;
	switch (value.type) {
		case "WORKER_READY":
...
function frameIdentity(frame: ProtocolEnvelope): ProtocolIdentity {
	return {
...
function envelope(identity: ProtocolIdentity): ProtocolEnvelope {
	return {
...
async function sendOverParentIpc(frame: ChildFrame): Promise<void> {
	if (!process.send || !process.connected) throw new Error("Parent IPC is unavailable.");
	if (!isControlMessageWithinLimit(frame)) throw new Error("Guardian frame exceeds the control-message limit.");
...
async function sendOverWorkerIpc(worker: ChildProcess, frame: ParentFrame): Promise<void> {
	if (!worker.send || !worker.connected) throw new Error("Worker IPC is unavailable.");
	if (!isControlMessageWithinLimit(frame)) throw new Error("Worker frame exceeds the control-message limit.");
	await new Promise<void>((resolveSend, rejectSend) => {
...
async function requireDirectory(path: string): Promise<void> {
	if (!isAbsolute(path)) throw new Error("Expected an absolute directory path.");
	const metadata = await stat(path);
	if (!metadata.isDirectory()) throw new Error("Expected a directory.");
}

async function requireRegularFile(path: string): Promise<void> {
	if (!isAbsolute(path)) throw new Error("Expected an absolute file path.");
	const metadata = await stat(path);
	if (!metadata.isFile()) throw new Error("Expected a regular file.");
}

async function captureSpoolIdentity(path: string): Promise<SpoolIdentity> {
	if (!isAbsolute(path)) throw new Error("Spool path must be absolute.");
	const canonical = await realpath(path);
	if (canonical !== resolve(path)) throw new Error("Spool path must not contain symlinks.");
	const metadata = await lstat(canonical);
	if (!metadata.isDirectory() || metadata.isSymbolicLink()) throw new Error("Spool path must be a real directory.");
	if ((metadata.mode & 0o777) !== 0o700) throw new Error("Spool directory mode must be 0700.");
...
async function removeCapturedSpool(identity: SpoolIdentity): Promise<void> {
...
async function resolveWorkerPath(bootstrap: BootstrapFrame): Promise<string> {
	...
	if (
		!isNonEmptyString(configuredRoot) ||
...
async function validateBootstrapPaths(bootstrap: BootstrapFrame): Promise<SpoolIdentity> {
	await Promise.all([
...
export class GuardianRuntime {
	private phase: GuardianPhase = "BOOTSTRAP";
	private identity: ProtocolIdentity | undefined;
	private bootstrap: BootstrapFrame | undefined;
	private spoolIdentity: SpoolIdentity | undefined;
	private worker: ChildProcess | undefined;
	private workerIdentity: WorkerIdentity | undefined;
	private workerExitPromise: Promise<WorkerExit> | undefined;
	private resolveWorkerExit: ((exit: WorkerExit) => void) | undefined;
	private workerReady = false;
	private workerExitSent = false;
	private parentConnected = true;
	private lease: Socket | undefined;
	private parentInbound = Promise.resolve();
	private workerInbound = Promise.resolve();
	private parentOutbound = Promise.resolve();
	private workerOutbound = Promise.resolve();
	private cleanup: Promise<void> | undefined;
	private donePromise: Promise<number>;
	private resolveDone!: (exitCode: number) => void;
	...
	async run(): Promise<number> {
		if (!process.send) throw new Error("Guardian requires an inherited Parent IPC channel.");
		this.attachLease();
	...
	private readonly onLeaseClosed = (): void => {
	...
	private readonly onParentMessage = (value: unknown): void => {
	...
	private async handleParentMessage(value: unknown): Promise<void> {
		if (this.phase === "DONE" || this.phase === "LOSS_CLEANUP" || this.phase === "SHUTDOWN") return;
	...
	private async bootstrapWorker(frame: BootstrapFrame): Promise<void> {
		...
		const ready: GuardianReadyFrame = {
			type: "GUARDIAN_READY",
	...
	private readonly onWorkerMessage = (value: unknown): void => {
	...
	private async handleWorkerMessage(value: unknown): Promise<void> {
		if (this.phase === "DONE" || this.phase === "LOSS_CLEANUP" || this.phase === "SHUTDOWN") return;
		...
		if (value.type === "WORKER_READY") throw new Error("Duplicate Worker handshake.");
		if (value.type === "PROTOCOL_ERROR") throw new Error("Worker rejected the control protocol.");
		await this.queueParentFrame(value);
	}
	...
	private readonly onWorkerDisconnect = (): void => {
		if (this.phase === "DONE" || this.phase === "SHUTDOWN") return;
		void this.startLossCleanup(false);
	};

	private readonly onWorkerExit = (code: number | null, signal: NodeJS.Signals | null): void => {
	...
	private queueParentFrame(frame: ChildFrame): Promise<void> {
		const operation = this.parentOutbound.then(async () => {
			if (!this.parentConnected || !process.connected) return;
			try {
	...
	private queueWorkerFrame(frame: ParentFrame): Promise<void> {
	...
	private async sendWorkerExit(exit?: WorkerExit): Promise<void> {
		if (this.workerExitSent || !this.identity) return;
	...
	private async observeWorkerExit(): Promise<WorkerExit | undefined> {
		const worker = this.worker;
		if (worker && (worker.exitCode !== null || worker.signalCode !== null)) {
			return { code: worker.exitCode, signal: worker.signalCode };
		}
		if (!this.workerExitPromise) return undefined;
		return await Promise.race([this.workerExitPromise, delay(WORKER_EXIT_OBSERVATION_MS).then(() => undefined)]);
	}

	private async startLossCleanup(protocolFailure: boolean, exit?: WorkerExit): Promise<void> {
		if (this.phase === "DONE" || this.phase === "SHUTDOWN") return;
	...
	private async terminateWorkerGroupForLoss(pgid: number): Promise<void> {
		if (!processGroupExists(pgid)) return;
		signalProcessGroup(pgid, "SIGTERM");
		if (await waitForProcessGroupExit(pgid, Date.now() + LOSS_TERM_GRACE_MS)) return;
	...
	private async startShutdown(requestedDeadlineAt: number): Promise<void> {
		if (this.phase === "DONE" || this.phase === "LOSS_CLEANUP") return;
		if (this.cleanup) return await this.cleanup;
		this.phase = "SHUTDOWN";
		const now = Date.now();
		...
		this.cleanup = (async () => {
			try {
				if (this.worker && this.identity) {
					const shutdownFrame: ShutdownFrame = {
						type: "SHUTDOWN",
						...envelope(this.identity),
						deadlineAt,
					};
				...
				if (this.spoolIdentity) await removeCapturedSpool(this.spoolIdentity);
				if (this.identity) {
					await this.queueParentFrame({
	...
	private async terminateWorkerGroupForShutdown(pgid: number, deadlineAt: number): Promise<void> {
		if (!processGroupExists(pgid)) return;
		const cooperativeDeadline = Math.max(
			Date.now(),
			Math.min(Date.now() + COOPERATIVE_PHASE_MS, deadlineAt - (TERM_PHASE_MS + KILL_PHASE_MS)),
		);
		if (await waitForProcessGroupExit(pgid, cooperativeDeadline)) return;

		signalProcessGroup(pgid, "SIGTERM");
		const termDeadline = Math.max(Date.now(), deadlineAt - KILL_PHASE_MS);
		if (await waitForProcessGroupExit(pgid, termDeadline)) return;

		signalProcessGroup(pgid, "SIGKILL");
		if (await waitForProcessGroupExit(pgid, deadlineAt)) return;
		throw new Error("Worker process group did not become empty after shutdown SIGKILL.");
	}

	private async handleParentProtocolFailure(): Promise<void> {
		if (this.phase === "DONE") return;
...
export async function runGuardianProcess(): Promise<number> {
